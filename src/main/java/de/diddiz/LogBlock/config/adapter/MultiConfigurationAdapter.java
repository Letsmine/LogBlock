...
public class MultiConfigurationAdapter implements ConfigurationAdapter {
     ...
     */
    public MultiConfigurationAdapter(LogBlock plugin, List<ConfigurationAdapter> adapters) {
        this.plugin = plugin;
        this.adapters = ImmutableList.copyOf(adapters).reverse();
    }

    public MultiConfigurationAdapter(LogBlock plugin, ConfigurationAdapter... adapters) {
    ...
    public Map<String, Map<String, String>> getSectionStringMap(String path, Map<String, Map<String, String>> def) {
        ...
        for (ConfigurationAdapter adapter : this.adapters) {
            result = adapter.getSectionStringMap(path, result);
        }
